...
uint256 hashGenesisBlock("0x3d3a2b10cb3f91fe29ea14e2df80599fbce1519eff18dab571a35020f0ab8d85");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GoalCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GoalCoin Signed Message:\n";

double dHashesPerSec;
int64 nHPSTimerStart;

// Settings
int64 nTransactionFee = 50;
...
int64 GetGOALSubsidy(int nHeight) {
...
int64  GetBlockValue(int nHeight, int64 nFees) {
   ...
   if(nHeight <= nDiffChangeTarget) {
      //this is pre-mining, reward is 230000.
      nSubsidy = 230000 * COIN;
      
   } else {
      //patch takes effect after 1 blocks solved
      nSubsidy = GetGOALSubsidy(nHeight);
   }

   //make sure the reward is at least 0.03 GOAL
   if(nHeight > 4213440) {
      nSubsidy = 0.03 * COIN;
   }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
   ...
   if(fNewDifficultyProtocol) {
    ...
    // GoalCoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal goalcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GoalCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3d3a2b10cb3f91fe29ea14e2df80599fbce1519eff18dab571a35020f0ab8d85");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Atletico Madrid gana la liga LFP BBVA 2014 contra Barcelona";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xcbdd11268ea839e6d2d823c473127841017e6d7db8662a03ae96c3826b6c40bb"));
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // GoalCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
